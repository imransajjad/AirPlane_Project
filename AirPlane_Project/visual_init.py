...
# ground
ground_box = box(size=(300*200,0.1,300*200),\
...
# draw airplane
a_plane_fus = cone(radius=0.2, color=color.red);
a_plane_wing_l = box(make_trail=True, size=(3,0.1,4), color=color.red);
a_plane_wing_r = box(make_trail=True, size=(3,0.1,4), color=color.red);
a_plane_wing_r.trail_object.color = color.blue;
a_plane_tail = box(size=(2,0.5,0.01) , color=color.blue);
...
hud_tar1 = ring(pos=(a_pos), axis=a_vel, radius=0.3, thickness=0.02, \
                material=materials.unshaded, color=color.red, linecolor=color.red)
hud_tar2 = curve(pos=(a_pos), axis=a_vel, thickness=0.05, \
                material=materials.unshaded, color=color.red, linecolor=color.red)
